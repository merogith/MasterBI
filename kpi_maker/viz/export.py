...
# Print renders always use the light palette: paper has no dark mode.
PRINT = TOKENS["light"]
...
def _prepare_for_print(spec: ChartSpec, width: int, height: int):
    ...
    fig.update_layout(
        width=width,
        height=height,
        paper_bgcolor=PRINT["surface"],
        plot_bgcolor=PRINT["surface"],
...
def render_all(specs: List[ChartSpec], out_dir: Optional[Path] = None,
    ...
    for spec in specs:
        try:
            png = render_png(spec, width=width)
